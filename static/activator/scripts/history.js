	...
	$.ajax("/activator/get_history/", {
		...
		success: function(data){
			if(data['report'] == 'error') {
				...
				$('#logs').html('<thead><tr><th class="text-center" scope="col" style="width: 10%">user</th><th class="text-center" scope="col" style="width: 75%">message</th><th class="text-center" scope="col" style="width: 15%">time</th></tr></thead>');
				...
				for (var i = data['logs'].length - 1; i >= 0; i--) {
					$('#logs_table').append(`<tr><td>${data['logs'][i][2]}</td><td>${data['logs'][i][3]}</td><td>${data['logs'][i][4]}</td></tr>`);
